...
/// 构造一张测试图：全黑、alpha=1；putBlock 时在左上角放一个 20x20 红块。
...
void main() {
  test('正向映射：整图选区保持红块面积比例', () {
    ...
    final out = exportIdPhoto(p, crop, whiteBg: false);
  ...
  test('白底合成：非红区域应为白色而非黑色', () {
    final p = makeImage(200, 100, putBlock: true);
    final crop = CropRect(100, 50, 200, 100);
    final out = exportIdPhoto(p, crop, whiteBg: true);
    // 背景区域应为白
  ...
  test('集成：图片与选区一起顺时针旋转，红块落点正确', () {
    ...
    final out = exportIdPhoto(pCW, crop, whiteBg: false);
  ...
  test('集成：图片与选区一起逆时针旋转，红块落点正确', () {
    ...
    final out = exportIdPhoto(pCCW, crop, whiteBg: false);
